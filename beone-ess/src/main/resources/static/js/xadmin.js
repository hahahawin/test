;!function (win) {
  ...
  Xadmin.prototype.end = function() {
	...
	Xadmin.prototype.add_tab = function (title,url,is_refresh) {
        ...
        var h = url.substr(0,url.lastIndexOf("&CODE"));
        h=h.substr(h.lastIndexOf("&ID=")+4);    //截取到当前菜单的id

        var bzzx='';
        if (JxCore.helpInfo[id]==null||JxCore.helpInfo[id]==""||JxCore.helpInfo[id].length==0||JxCore.helpInfo[id]==undefined){
            bzzx = getHelpNr(h);
            JxCore.helpInfo[id]=bzzx;
        }else{
            bzzx = JxCore.helpInfo[id];
        }
        $("#helpConten",parent.parent.document).html(bzzx);
  ...
  Xadmin.prototype.del_tab = function (id) {
	...
	Xadmin.prototype.add_lay_tab = function(title,url,id) {
		element.tabAdd('xbs_tab', {
	       title: title 
	        ,content: '<iframe tab-id="'+id+'" frameborder="0" src="'+url+'" title="'+title+'" scrolling="yes" class="x-iframe"></iframe>'
	        ,id: id
	    })
	}
	/**
...
layui.use(['layer','element','jquery'],function() {
    ...
    element.on('tab(xbs_tab)', function(data){
        ...
        $("#helpConten",parent.parent.document).html(JxCore.helpInfo[id]);
    });
    ...
    $('.container .left_open i').click(function(event) {
        if($('.left-nav').css('width')=='220px'){
            ...
            $('#navTitle')[0].innerHTML="<span onclick=\"closeNav()\" class=\"iconfont\"><i id=\"navButton\" class=\"iconfont\" style=\"margin:10px 15px 10px 15px;\"><img style=\"padding-top: 10px\" src=\"img/icon/sidebar_icon_shrink2.png\"></i></span>";
            ...
            $('#navTitle')[0].innerHTML="<cite>- 菜单导航栏 -</cite>\n" +
                "                <span onclick=\"closeNav()\" class=\"iconfont\"><i id=\"navButton\" class=\"iconfont\" style=\"float: right\"><img style=\"padding-top: 10px\" src=\"img/icon/sidebar_icon_shrink2.png\"></i></span>";
    ...
    $('#tab_right').on('click', 'dd', function(event) {
        ...
        if(data_type=='this'){
          ...
          $('.layui-tab-title li[lay-id]').find('.layui-tab-close').click();
        }else if(data_type=='reload'){
            var src = $(".layui-tab-content .x-iframe[tab-id='"+lay_id+"']").attr("src");
            var title = $(".layui-tab-content .x-iframe[tab-id='"+lay_id+"']").attr("title");
            // console.info(lay_id+":"+src+","+title);
            xadmin.add_tab(title,src,true);
        }
        $('#tab_right').hide();
        $('#tab_show').hide();
    })
...
function getLeftOnClick() {
    ...
    $('.left-nav #nav').on('click', 'li', function(event) {
        ...
        var i=0;
        $("#nav").find('a').find('i').find('img').each(function(){
            var url = $("#nav").find('a').find('i').find("img").eq(i).attr("src");
        ...
        if($(this).parent().attr('id')=='nav'){
            var url1 = $(this).find('a').find('i').find('img').eq(0).attr("src");
            url1 =  url1.replace('gray','blue');
            $(this).find('a').find('i').find("img").eq(0).attr("src", url1);
            ...
            var url1 = $(this).find('a').find('i').find('img').attr("src");
            url1 =  url1.replace('gray','blue');
            $(this).find('a').find('i').find("img").attr("src", url1);
        }
...
$(function () {
    $(".page-content .layui-tab .layui-tab-title").on("click","li",function () {
        var id = $(this).attr("lay-id");
        JxCore.helpInfo[id]
    })
})*/
