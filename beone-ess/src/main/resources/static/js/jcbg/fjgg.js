...
$(function () {
    ...
    layui.use(['table','form','laydate','upload'], function() {
        ...
        var Table={
            ...
            limit:10,
                ...
                $("#YWID").empty();
                $("#YWID").append("<option value=''>请选择房间</option>");
                for(var j in fjList){
                    $("#YWID").append("<option value='"+j+"'>"+fjList[j]+"</option>");
                }
                form.render();
            },
            where:json, //接口的其它参数
            cols: [[ //表头
                ...
                ,{field: 'YWID', title: '房间',  sort: false,templet:function (obj) {
                        return fjList[obj.YWID]
                    }}
                ...
                , {title:'操作',width:200 , templet:function (obj) {//toolbar:'#barConfig'
                        ...
                        var judge={};
                        judge.JCBG_TZGG_FJGG_EDIT=pd;
                        judge.JCBG_TZGG_FJGG_DEL=pd;
                        judge.JCBG_TZGG_FJGG_ZF=pd;
                        json2.judge=judge;
        ...
        table.on('tool(conTable)', function(obj){
            if(obj.event === 'JCBG_TZGG_FJGG_DEL'){
            ...
            } else if(obj.event === 'JCBG_TZGG_FJGG_EDIT'){
                BjOrXz="BJ";
                status="";
                loadForm();
                ...
                form.val('form', {
                    ...
                    "JJCD":obj.data.JJCD,
                    "YWID":obj.data.YWID,
                    "SXSJ":dateFormat("yyyy-mm-dd",obj.data.SXSJ),
                    "TXFS":obj.data.TXFS,
                });
                ...
                form.render();
            }else if(obj.event === 'JCBG_TZGG_FJGG_ZF'){
        ...
        table.on('rowDouble(conTable)',function (obj) {
            ...
            $("#configForm div[name='FJ']").html(fjList[data.YWID]);
    ...
    function getFjList() {
        ...
        var json={};
        json.ASD=jsonASD;
        getAjax({url:getFjListUrl,data:JSON.stringify(json),callback:function (reg) {
            var list = reg.resultData;
                for (var i=0;i<list.length;i++){
                    fjJson[list[i].id]=list[i].mc;
                }
            }
        });
        return fjJson;
