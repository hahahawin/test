...
$(function () {
    ...
    layui.use(['table','form'], function() {
        ...
        var Table={
            ...
            where:json, //接口的其它参数
            cols: [[ //表头
                {field: 'NAME', title: '组织名称',  sort: false}
                , {field: 'CODE', title: '组织编码',  sort: false,}
                // , {field: 'PID',width:100, title: '上级组织',  sort: false}
                , {field: 'BJZT',width:100, title: '编辑状态',  sort: false
        ...
        table.on('toolbar(conTable)', function(obj){
            switch(obj.event){
                ...
                case 'findOnCondition':
                    if($("#org_name").val()!=""){
                        json.org_name=$("#org_name").val();
                    }else{
                        delete json.org_name;
                    }
                    table.init('conTable', Table);
                    $("#org_name").val(json.org_name);
                    break;
            };
        });
